...
inline void crearPila(pilasFicha &pila) {
    // Le pasamos la pila por referencia (&) para poder modificarla
    pila.tope = nullptr;
}
...
// PUSH - mete los elementos por cabeza a la pila
...
// POP - saca los elementos de la pila 
inline Ficha sacarFichaPila(pilasFicha &pila) {
    if (pilaVacia(pila)) {
        // devolvemos una ficha "mala" para evitar que el programa crashee
